...
if st.session_state.file_loaded and st.session_state.df_dict:
    ...
    with tab1:
        if not demo.empty:
            st.dataframe(demo.head(10), use_container_width=True, hide_index=True)
    with tab2:
        if not med.empty:
            st.dataframe(med.head(10), use_container_width=True, hide_index=True)
    with tab3:
        if not lab.empty:
            st.dataframe(lab.head(10), use_container_width=True, hide_index=True)
