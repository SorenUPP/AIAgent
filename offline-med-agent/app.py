...
if not st.session_state.get("auth_user"):
    ...
    pg = st.navigation([
        st.Page("views/0_Console.py", title="Console", icon=":material/terminal:", default=True),
        st.Page("views/1_dashboard.py", title="Dashboard", icon=":material/dashboard:"),
        st.Page("views/2_Patient_Explorer.py", title="Patient Explorer", icon=":material/groups:"),
        st.Page("views/3_Audit_Log.py", title="Audit Log", icon=":material/fact_check:"),
    ])
